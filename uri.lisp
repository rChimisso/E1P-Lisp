...
(defmethod setv ((m machine) &key userinfo host (port (port m)) path quer frag)
  (setf
   (values
    (userinfo m)
    (host m)
    ...
    (fragment m)
    (valid m))
   (values userinfo host port path quer frag t)))
...
(defmethod recur-parse ((m machine)
                        leftover
                        packs
                        &key
                        authority
...
(defmethod parse ((m machine))
  (cond
   ...
   (t (recur-parse m (leftover m) '("AUTHORITY" "PATH" "QUERY" "FRAGMENT"))))
  (if (valid m)
      (setv m
